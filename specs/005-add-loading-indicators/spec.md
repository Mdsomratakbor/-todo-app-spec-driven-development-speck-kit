...
When a user opens any screen that loads data (todo list, categories, lunch settings, dashboard), they see lightweight skeleton placeholders shaped like the real content instead of a blank page or a single spinner. As data arrives, the skeletons smoothly transition into the actual content.
...
Every operation the user initiates that involves waiting — submitting a form, saving a todo, deleting a category, saving lunch settings, logging in, logging out — visibly indicates progress at the point of action. The triggering control (e.g., button) shows a busy state, is disabled against duplicate submission, and returns to normal when the operation finishes.
...
1. **Given** an operation completes quickly (under a defined threshold), **When** it starts and finishes, **Then** no flickering loader is shown
...
- What happens on retry after a failed load? (loader reappears and resolves or fails again)
...
- **FR-005**: When an operation fails, the loading indicator MUST be removed, the control MUST be re-enabled, and an error notification MUST be shown with retry where appropriate
...
- **FR-007**: Loading indicators MUST be suppressed (no flicker) for operations that complete within a defined fast-operation threshold
...
- **SC-003**: Replacing skeletons with real content causes no more than a small, imperceptible layout shift (content appears in the skeleton's position)
...
- **SC-005**: Operations completing under the fast-operation threshold display no visible loading flash (verified by screen capture or manual review)
...
- A "fast-operation threshold" of roughly 300 ms is a reasonable default; operations completing faster than this should not flash a loader
